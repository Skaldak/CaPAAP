...
def iterate_invert_spectrogram(X_s, fftsize, step, n_iter=10, verbose=False):
    ...
    for i in range(n_iter):
        ...
        if i == 0:
            X_t = invert_spectrogram(X_best, step, calculate_offset=True, set_zero_phase=True)
        else:
    ...
    for i in range(X_s.shape[0]):
        ...
        if calculate_offset and i > 0:
            ...
            if offset_size <= 0:
                ...
                offset_size = step
            offset = xcorr_offset(
                wave[wave_start : wave_start + offset_size], wave_est[est_start : est_start + offset_size]
            )
        else:
            offset = 0
...
def make_mel(spectrogram, mel_filter, shorten_factor=1):
    ...
    mel_spec = scipy.ndimage.zoom(mel_spec.astype("float32"), [1, 1.0 / shorten_factor]).astype("float16")
...
def create_mel_filter(fft_size, n_freq_components=64, start_freq=300, end_freq=8000, samplerate=44100):
    ...
    mel_inversion_filter = get_filterbanks(
        nfilt=n_freq_components, nfft=fft_size, samplerate=samplerate, lowfreq=start_freq, highfreq=end_freq
    )
    # Normalize filter
    ...
    def __init__(self, root_dir, labels, spectogram_step, spectogram_freqs, frame_step, frame_size, traintest="TRAIN"):
        ...
        for dirName, subdirList, fileList in os.walk(os.path.join(root_dir, traintest)):
            for fname in fileList:
                ...
                self.spectogram = pretty_spectrogram(
                    data.astype("float64"), fft_size=spectogram_freqs, step_size=spectogram_step
                )
