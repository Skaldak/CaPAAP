...
def train(model, optimizer, train_loader, epoch):
    ...
    for batch_id, (data, target) in enumerate(tqdm(train_loader)):
        ...
        if batch_id % 100 == 0:
            tqdm.write(
                "Epoch: [{}/{}], Batch: [{}/{}], train accuracy: {:.6f}, loss: {:.6f}".format(
                    epoch, N_EPOCHS, batch_id + 1, n_batch, correct / float(BATCH_SIZE), train_loss / float(BATCH_SIZE)
                )
            )
...
def test(capsule_net, test_loader, epoch):
    ...
    tqdm.write(
        "Epoch: [{}/{}], test accuracy: {:.6f}, loss: {:.6f}".format(
            epoch, N_EPOCHS, correct / len(test_loader.dataset), test_loss / len(test_loader)
        )
    )
