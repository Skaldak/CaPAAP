...
class DataCollatorNFCWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        batch["mask_time_indices"] = _compute_mask_indices(
            (batch_size, sequence_length), mask_prob=0.1, mask_length=2, device="cpu"
        )
...
class SpeechCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        batch["mask_time_indices"] = _compute_mask_indices(
            (batch_size, sequence_length), mask_prob=mask_prob, mask_length=2, device="cpu"
        )
...
class CVCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        # get speech features
        input_features = [
            {"input_values": audio_preprocess(feature["path"].replace(".mp3", ".wav"))} for feature in features
        ]
        mel_len = [model._get_feat_extract_output_lengths(len(i["input_values"])) for i in input_features]
        batch = self.processor.pad(
            input_features,
