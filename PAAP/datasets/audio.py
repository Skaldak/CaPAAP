...
def get_egemap(file, file_length, examples, output_path, smile, length, stride, sample_rate, level):
    ...
    for seg_idx in range(examples):
        ...
        if torchaudio.get_audio_backend() in ["soundfile", "sox_io"]:
            seg, sr = torchaudio.load(str(file), frame_offset=offset, num_frames=num_frames or -1)
        else:
...
def execute_multiprocess(files, num_examples, output_path, smile, length, stride, sample_rate, level):
    ...
    with multiprocessing.Pool(PROCESSES) as pool:
        ...
        in_args = [
            (file, file_length, examples, output_path, smile, length, stride, sample_rate, level)
            for (file, file_length), examples in zip(files, num_examples)
...
def execute_multiprocess_spec(files, num_examples, output_path, spectrogram, length, stride, sample_rate):
    ...
    with multiprocessing.Pool(PROCESSES) as pool:
        ...
        in_args = [
            (file, file_length, examples, output_path, spectrogram, length, stride, sample_rate)
            for (file, file_length), examples in zip(files, num_examples)
            if not os.path.exists(os.path.join(output_path, os.path.basename(file.replace(".wav", ".npy"))))
        ]
...
class Audioset:
    def __init__(
        ...
        if self.stride is not None:
            self.is_random_sampling = (
                self.stride >= self.files[0][1]
            )  # TODO: Here it assumes all of the files are of the same length
        else:
        ...
        if acoustic_path is not None:
            ...
            if len(glob(os.path.join(acoustic_path, "*.npy"))) < len(self.files):
                print(
                    "eGeMAPS LLDs do not exist (%d/%d). Generating... This might take a while"
                    % (len(glob(os.path.join(acoustic_path, "*.npy"))), len(files))
                )
                import opensmile
        ...
        if ph_logits_path is not None:
            ...
            if len(glob(os.path.join(ph_logits_path, "*.npy"))) < len(self.files):
                print(
                    "phoneme logits do not exist (%d/%d). Generating... This might take a while"
                    % (len(glob(os.path.join(ph_logits_path, "*.npy"))), len(files))
                )
                execute_singleprocess_ph_logits(
                    self.files, self.num_examples, ph_logits_path, self.length, self.stride, self.sample_rate
                )
        return
    ...
    def __getitem__(self, index, frame_offset=None):
        for (file, file_length), examples in zip(self.files, self.num_examples):
            ...
            if self.acoustic_path is not None:
                acoustics = torch.from_numpy(
                    np.load(os.path.join(self.acoustic_path, os.path.basename(file.replace(".wav", ".npy"))))
                ).float()
                acoustics = F.normalize(acoustics)
            else:
                acoustics = torch.Tensor([-1])
            if self.ph_logits_path is not None:
                ph_logits = torch.from_numpy(
                    np.load(os.path.join(self.ph_logits_path, os.path.basename(file.replace(".wav", ".npy"))))
                ).float()
            else:
