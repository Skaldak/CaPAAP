...
class FullSubNet(BaseModel):
    ...
    def forward(self, noisy, clean=None):
        ...
        if clean is not None:
            ...
            if noisy.size(0) > 1:
                cIRM = drop_band(cIRM.permute(0, 3, 1, 2), self.num_groups_in_drop_band).permute(  # [B, 2, F ,T]
                    0, 2, 3, 1
                )
    ...
    def forward_noisy_mag(self, noisy_mag):
        ...
        sb_input = sb_input.reshape(
            batch_size * num_freqs, (self.sb_num_neighbors * 2 + 1) + (self.fb_num_neighbors * 2 + 1), num_frames
        )
