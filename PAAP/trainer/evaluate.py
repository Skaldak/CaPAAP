...
parser.add_argument("--no_pesq", action="store_false", dest="pesq", default=True, help="Don't compute PESQ.")
...
def evaluate(args, model=None, data_loader=None):
    ...
    if data_loader is None:
        dataset = NoisyCleanSet(args.data_dir, matching=args.matching, sample_rate=model.sample_rate)
    ...
    with ProcessPoolExecutor(args.num_workers) as pool:
        with torch.no_grad():
            ...
            for i, data in enumerate(iterator):
                ...
                if args.device == "cpu":
                    pendings.append(pool.submit(_estimate_and_run_metrics, clean, model, noisy, args))
                    # estimate = get_estimate(model, noisy, args)
                else:
                    estimate = get_estimate(model, noisy, args)
                    estimate = estimate.cpu()
                    clean = clean.cpu()
                    pendings.append(pool.submit(_run_metrics, clean, estimate, args, model.sample_rate))
                """
