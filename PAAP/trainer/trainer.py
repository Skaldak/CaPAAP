...
class Trainer(object):
    def __init__(self, data, model, optimizer, args):
        ...
        self.args = args
        self.mrstftloss = MultiResolutionSTFTLoss(factor_sc=args.stft_sc_factor, factor_mag=args.stft_mag_factor).to(
            self.device
        )
            ...
            if continue_pretrained is not None:
                ...
                if self.args.model == "Demucs":
                    ...
                    state_dict = torch.load(
                        continue_pretrained, map_location=torch.device(torch.cuda.current_device())
                    )["model"]
                    self.model.load_state_dict(state_dict)
                else:
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        return_stuff = {
            "total_loss": distrib.average([total_loss / (i + 1)], i + 1)[0],
